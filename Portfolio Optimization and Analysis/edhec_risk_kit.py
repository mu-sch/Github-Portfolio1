...
def skewness(r):
    ...
    demeaned_r = r - r.mean()
    # use the population standard deciation, so set dof = 0
...
def minimize_vol(target_return, er, cov):
    ...
    return_is_target = {
        'type': 'eq',
        'args': (er,),
        'fun': lambda weights, er: target_return - portfolio_return(weights, er) # Lambda/Anonymous function. not defined outside the block
    }
    weights_sum_to_1 = {
        'type': 'eq',
...
def optimal_weights(n_points, er, cov): # defining optimal_weights in 'minimize_vol'
    """
...
def msr(riskfree_rate, er, cov):
    ...
    given the riskfree rate and expected returns and a covariance matric
    """
    n = er.shape[0]
    init_guess = np.repeat(1/n, n)
    bounds = ((0.0, 1.0),)*n    # Tuple of tuples
...
def run_cppi(risky_r, safe_r=None, m=3, start=1000, floor=0.8, riskfree_rate=0.03, drawdown=None):
    ...
    Returns a dictionary containing: Asset Value History, Risk Busget History, Risky Weight History
    """
    ...
    if isinstance(risky_r, pd.Series): # So that it will initialize a DataFrame from a series so it works with series as well
    ...
    # For Running a backtest of the algorithm over time
    ...
    for step in range(n_steps):
        ...
        risky_w = m*cushion
        risky_w = np.minimum(risky_w, 1) # Dont go above 100% allocation(no leverage)
        risky_w = np.maximum(risky_w, 0) # Don't go below 0% allocation
        safe_w = 1-risky_w
        # Now we know our risky/safe allocations
        risky_alloc = account_value*risky_w
        safe_alloc = account_value*safe_w
        # Recompute the account value for this time step
        account_value = risky_alloc*(1+risky_r.iloc[step]) + safe_alloc*(1+safe_r.iloc[step])
        # Save the values for analysis and plotting, etc
...
def discount(t, r):
    ...
    and r is the per-period return interest rate
...
def bond_price(maturity, principal=100, coupon_rate=0.03, coupons_per_year=12, discount_rate=0.03):
    ...
    i.e. The index of the discount_rate DataFrame is assumrf to be the other coupon number
    """
...
# How to write and backtest strategies that construct portfolios that mix psp and ghp(goal-hedging portfolios) in a 
# systematic way

# backtester 
def bt_mix(r1, r2, allocator, **kwargs):
    """
    Runs a back test(simulation) of allocating between 2 sets of returns r1 and r2 which are T x N DataFrames
    (Rows are T(times) Columns are N(scenarios)
    or returns where T is the time step index and N is the number of scenarios.
    allocator is a function that takes two sets of returns and allocator specific parameters, and produces
    an allocation to the first portfolio (the rest is invested in the GHP) as a T x 1 DataFrame
    Returns a T x N DataFrame of the resulting N portfolio scenarios
    """
    if not r1.shape == r2.shape:
        raise ValueError("r1 and r2 need to be the same shape dumbass")
...
def glidepath_allocator(r1, r2, start_glide=1, end_glide=0):  #start,end--> where you want the % split to start and end from
    """
...
def floor_allocator(psp_r, ghp_r, floor, zc_prices, m=3):
    """
    Allocate between PSP and GHP with the goal to provide more exposure to the upside
    of the PSP without violating the floor.
    Uses a CPPI-style dynamic risk budgeting algorithm by investing a multiple
    of the cushion in the PSP.
    Returns a DataFrame with the same shape as the psp/ghp representing weights in the PSP
    """
    ...
    w_history = pd.DataFrame(index=psp_r.index, columns=psp_r.columns) # same shape as psp
    for step in range(n_steps):
        floor_value = floor*zc_prices.iloc[step]                   ## PV of Floor assuming todays rates and flat yield curve
        cushion = (account_value - floor_value)/account_value       # same as cppi cushion
        psp_w = (m*cushion).clip(0, 1)                              # same as applying max and min
        ghp_w = 1-psp_w
...
def drawdown_allocator(psp_r, ghp_r, maxdd, m=3):
    ...
    for step in range(n_steps):
        ...
        psp_alloc = account_value*psp_w
        ghp_alloc = account_value*ghp_w
        # recompute the new account value and the previous peak at the end of this step
        account_value = psp_alloc*(1+psp_r.iloc[step]) + ghp_alloc*(1+ghp_r.iloc[step])
...
def terminal_values(rets):
    """
    Returns the final values of $1 at the end of the return period for each scenario
    """
    return (rets+1).prod()
...
def terminal_stats(rets, floor=0.8, cap=np.inf, name="Stats"):
    ...
    breach = terminal_wealth < floor  # breaching below the  floor...will get a series of booleans, trues and falses
    reach = terminal_wealth >= cap    # Did i reach my cap?
    p_breach = breach.mean() if breach.sum() > 0 else np.nan # probability of a breach
    p_reach = breach.mean() if reach.sum() > 0 else np.nan   # probability of reaching cap
    e_short = (floor-terminal_wealth[breach]).mean() if breach.sum() > 0 else np.nan  # The average % of breach for the cases BELOW the floor
